...
def run_tests() -> tuple[int, int]:
    ...
    try:
        ...
        passed = sum(int(n) for n in re.findall(r"(\d+)\s+passed", output))
        failed = sum(int(n) for n in re.findall(r"(\d+)\s+failed", output))
        errors = sum(int(n) for n in re.findall(r"(\d+)\s+error", output))
